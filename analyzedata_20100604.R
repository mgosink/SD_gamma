...
###         datamatrix <- read.csv(file = dataname,sep=',',header = T);
...
#cont_fitting <- function (datamatrix, sudocounts = 0.001){
#cont_fitting <- function (datamatrix, sudocounts = 0.01){
cont_fitting <- function (datamatrix, sudocounts = 0.1){
#cont_fitting <- function (datamatrix, sudocounts = 1.0){

       realdim <- dim(datamatrix);
       datarownames <- rownames(datamatrix);
       datacolnames <- colnames(datamatrix);
       nonzeroloc <- datamatrix != 0;
      
       datamatrix <- datamatrix + sudocounts;                
       con <- log(sum(sum(datamatrix)));
